$(document).ready(function () {
    ...
    $('#rowsPerPage').on('change', function () {
        rowsPerPage = parseInt($(this).val()) || 5;
        currentPage = 1;
        loadEmployees();
    });

    $(document).on('click', '.sort-up, .sort-down', function () {
    ...
    function applySorting(cache, column, order) {
        cache.sort((a, b) => {
            ...
            if (column === 'Name') return order === 'asc' ? a.employeeName.localeCompare(b.employeeName) : b.employeeName.localeCompare(a.employeeName);
            if (column === 'Gender') return order === 'asc' ? a.employeeGender.localeCompare(b.employeeGender) : b.employeeGender.localeCompare(a.employeeGender);
            if (column === 'Salary') return order === 'asc' ? a.employeeSalary - b.employeeSalary : b.employeeSalary - a.employeeSalary;
            if (column === 'Department') return order === 'asc' ? a.departmentName.localeCompare(b.departmentName) : b.departmentName.localeCompare(a.departmentName);
            if (column === 'Active') return order === 'asc' ? a.employeeActive - b.employeeActive : b.employeeActive - a.employeeActive;
            if (column === 'isDeleted') return order === 'asc' ? a.employeeDeleted - b.employeeDeleted : b.employeeDeleted - a.employeeDeleted;
            return 0;
        });
    }
    ...
    function loadEmployees() {
        const url = filter === 'active'
            ? `/Employee/GetActiveEmployeeList?showDeleted=${encodeURIComponent(showDeleted)}`
            : `/Employee/GetEmployeeList?showDeleted=${encodeURIComponent(showDeleted)}`;

        console.log("Loading employees...");  // should appear first

        $.get(url, function (employees, response) {
            console.log("Loaded employees", employees); // should appear later
            if (employees) {
    ...
    function renderEmployees(cache, page) {
        console.log("Rendering from cache", employeesCache); 
    ...
    // 🔥 Call it once DOM is ready and script is loaded
    loadEmployees();
});
